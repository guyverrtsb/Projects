    ...
    extends zMimesBaseObject
{
    ...
    function __construct($formfieldname = "GDUploadImageFile", 
                        $appl_db = "mimes_appl_stephen", 
                        $target_h_pxl = "500", 
                        $target_w_pxl = "500",
                        $target_image_size = "2097152")
    {
        $this->formfieldname = $formfieldname;
        $this->appl_db = $appl_db;
    ...
    function executeControl()
    {
        ...
        if($r == "VALID")
        {
            $uploadmime->setSaveFolderLocation(); $uploadmime->setSaveFileName();
            ...
            if($r == "FILE_UPLOADED_SUCCESSFULY")
            {
                ...
                if($r == "MIME_REGISTERED")
                {
                    ...
                    if($r == "MIME_BLOB_REGISTERED")
                    {
                        ...
                        $r = $zitdthmb->registerMime($thmbimg->getFileName(),
                                            $thmbimg->getType(),
                                            $thmbimg->getFileExt(),
                                            $thmbimg->getFileSize(),
                                            $thmbimg->getFilePath(),
                                            $thmbimg->getFileFolder(),
                                            "mimes_image_thumbnail_100x100",
                                            $_SESSION["GUYVERDESIGNS_SITE"],
                                            $_SESSION["GUYVERDESIGNS_SITE_ALIAS"]);
                        if($r == "MIME_REGISTERED")
                        {
                            ...
                            if($r == "MIME_BLOB_REGISTERED")
                            {
                                ...
                                $r = $zitdappl->registerMime($applimg->getFileName(),
                                                    $applimg->getType(),
                                                    $applimg->getFileExt(),
                                                    $applimg->getFileSize(),
                                                    $applimg->getFilePath(),
