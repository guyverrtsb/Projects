...
function load(startIdx, rowCount)
{
	...
	{
		SERVICE_CONTROL_KEY : "SEEMEU-GET_LIST_OF_UNIVERSITY_SOURCE"
	...
	function(data)
    {
		...
		data = eval("(" + data + ")");
		if(data.RETRIEVEENTITYUNIVERSITYSOURCE != null)
			gdBuildColleges(data.RETRIEVEENTITYUNIVERSITYSOURCE, "list_entityuniversitysource", 4);
		else
			endOfList = true;
    });
}
 ...
 * jsonObj = JSON Object
 */
function gdBuildColleges(data, elementId, numPerRow)
{
	/*
<div class="row">
<div class="col-lg-4">
...
<p id="gdsyslinksButton"><a class="btn btn-default" href="#" role="button">View details &raquo;</a></p>
    ...
    $.each(data, function(key, val)
    {
		...
		if(val.idx == 1)
		{
    		h2.text("Group Univ [" + val.idx + "]");
    		pcountent1.text(urlAry[(urlAry.length - 2)]);
    		pcountent2.text("");
    		pcountent3.text("");
        	a.attr("href", val.url).text(urlAry[(urlAry.length - 2)]);
		}
    	else if(urlAry.length == 9)
    	{
    		h2.text("College [" + val.idx + "]");
    		pcountent1.text(urlAry[(urlAry.length - 2)]);
    		pcountent2.text("");
    		pcountent3.text("");
        	a.attr("href", val.url).text("launch");
    	}
    	else if(urlAry.length == 8)
    	{
    		h2.text("Level 8 [" + val.idx + "]");
    		pcountent1.text(urlAry[(urlAry.length - 2)]);
    		pcountent2.text("");
    		pcountent3.text("");
        	a.attr("href", val.url).text("launch");
    	}
    	else if(urlAry.length == 7)
    	{
    		h2.text("Level 7 [" + val.idx + "]");
    		pcountent1.text(urlAry[(urlAry.length - 2)]);
    		pcountent2.text("");
    		pcountent3.text("");
        	a.attr("href", val.url).text("launch");
