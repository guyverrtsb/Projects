...
class SysIntegration
{
    ...
    {
        if (!isset($_SESSION[SysIntegration::getKeySessSiteUid()]) || !isset($_SESSION[SysIntegration::getKeySessSiteAliasUid()])
            || !isset($_SESSION[SysIntegration::getKeySessSite()]) || !isset($_SESSION[SysIntegration::getKeySessSiteAlias()]))
        {
            require_once("integration/siteandalias.php");
            //require_once("integration/roles.php");
            zLog()->LogInfo("_core.php:Session Object not Found");
            $siteandalias = new SiteandAlias();
            if(!$siteandalias->isSiteandAliasValid())  // Checkign to see if Site and Alias exist and are matched
            {
                zLog()->LogInfo("Site and Site Alias Not Valid");
                /*
                 ...
                 */
                 if(!$siteandalias->doesSiteExist())
                 {
                    zLog()->LogInfo("Site Not Found");
                    $siteandalias->registerSite();
                    // $zgdroles = new ZGDRoles();
                    // $zgdroles->registerRolestoNewSite();
                 }
        
                $siteandalias->registerSiteAlias();
                $siteandalias->matchSiteandSiteAlias();
            }
            else
            {
                zLog()->LogInfo("_config.php:Site and Site Alias Valid");
            }
        }
        else
        {
    ...
    static function dumpSessionData()
    {
        ...
        foreach ($_SESSION as $sessname => $sessvalue)
        {
            $syslog->LogInfo("SESSION:NAME-{".$sessname."}:VALUE-{".$sessvalue."}");
        }
    }
