...
<?php zReqOnce("/gd.trxn.com/usersafety/_controls/classes/accesspoints/createuser.php"); ?>
    ...
    extends AppSysBaseObject
{
    function execute()
    {
        ...
        if($this->validate())
        {
            ...
            $createuser->createUserInfo($args);
            if($createuser->getSysReturnCode() == "USER_IS_CREATED")
            {
                ...
                $cmeuu->basic($rua->getMatchEntitytoUniversityMatchEntityUid()
                            , $createuser->getSysReturnitem("useraccount_uid")
                            , "USER_TYPE-PROSPECT"
                            , "ENTITY_ROLE-USER");
                
                if($cmeuu->getSysReturnCode() == "RECORD_IS_CREATED" && zAppSysIntegration()->isLandscapeLocal())
                    $this->setSysReturnData("USER_IS_CREATED", "User is Created-".$createuser->getSysReturnitem("LCL_MSG_OVERRIDE"), "TRUE");
            ...
            else if($createuser->getSysReturnCode() == "EMAIL_IN_USE")
            {
                $this->setSysReturnData("EMAIL_IN_USE", "Email is already registered", "TRUE");
            }
            else if($createuser->getSysReturnCode() == "NICKNAME_IN_USE")
            {
                $this->setSysReturnData("NICKNAME_IN_USE", "Nickename in Use.  Try [".$createuser->getSysReturnitem("NICKNAME_SUGGESTION")."].", "TRUE");
            }
            else if($createuser->getSysReturnCode() == "USERTABLEKEY_IN_USE")
            {
