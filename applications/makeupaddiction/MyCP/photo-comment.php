...
$photo_id = intval($_REQUEST['photo']);

$where = " WHERE 1 AND PC.photo_id = '" . $photo_id . "'";
$PAGE_SIZE = 10;

if (isset($_REQUEST['mode']) && intval($_REQUEST['photo']) > 0 && $_REQUEST['mode'] == 'delete') {
    ...
    $sql = " DELETE FROM activity_log WHERE main_id = '" . $photo_id . "' AND ref_id = '" . $id . "' AND type = 'PHOTO' AND other_type = 'COMMENTPHOTO' ";
    $db->query($sql);
...
<div class="content-box"><!-- Start Content Box -->
    ...
    <div class="content-box-content">
        ...
        <div class="Registerinner">
            ...
            $sql = " SELECT PC.* ,P.* FROM photo_comment AS PC "
                    . " JOIN post_images AS P  ON "
                    . " PC.photo_id = P.image_id " . $where;
                ...
                <tbody>
                    ...
                    if ($resultCount > 0) {
                        ...
                        while ($row = $resultB->fetch()) {
                            $i++;
                            $photo = getPhoto($row['post_id']);

                            $path = $photo[0]['thumb_image'];

                            if ($path == '') {
                                $path = 'uploads/defalut_img@2x.jpg';
                            }
                            ...
                            <tr><td height="30px;" valign="top"><?php echo $i; ?></td>
                                ...
                                <td valign="top"><?php echo unEscapeChars($user['first_name'] . ' ' . $user['last_name']) ?></td>
                                ...
                                <td valign="top">
                                    <a href="<?php echo base_path ?>MyCP/photo-comment.php?photo=<?php echo $row['photo_id']; ?>&id=<?php echo $row['id']; ?>&mode=delete" onclick="return confirmDelete()" title="Delete"><img src="<?php echo base_path ?>MyCP/images/dd.gif" /></a>
                                </td>
