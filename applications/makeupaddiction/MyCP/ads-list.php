...
if (isset($_REQUEST['mode']) && intval($_REQUEST['id']) > 0 && $_REQUEST['mode'] == 'delete') {
    ...
    $sqlselect1 =   " SELECT * FROM ad_post WHERE ".
                    " id = '" . $id . "'";
    $queryse1 = $db->query($sqlselect1);
    $rsCount = $queryse1->size();

        if ($rsCount > 0) {
            $db->query("DELETE FROM ad_post where id = '" . $id . "' ");
    ...
    <div class="content-box-header">
       <h3 style="cursor: s-resize;">
        ...
        <span style="float: right;margin: 10px 10px ">
            <a href="<?php echo base_path ?>MyCP/ads-post.php"  class="inner-link" style="cursor:pointer;"> Add Ads</a>
        </span>
        <div class="clear"></div>
    </div>
            ...
            <table border="1" width="100%" id="managetable">
                <thead>
                    <tr>
                        ...
                        <th width="10%">Ads Title</th>
                        <th width="20%">Ads Text</th>
                        <th width="10%">Gender</th>
                        <th width="8%">Status</th>
                        <th width="20%">Schedules Date</th>
                ...
                <tbody>
                    ...
                    if ($resultCount > 0) {
                        ...
                        while ($row = $resultB->fetch()) {
							$sqlselect1 =   " SELECT * FROM member WHERE id = '" . $row['member_id'] . "'";
						    $queryse1 = $db->query($sqlselect1);
							$getmem = $queryse1->fetch();
                            $i++;
                            
                            ?>
                            <tr>
                                <td height="30px;" valign="top"><?php echo $i; ?></td>
                                <td valign="top"><img src="<?php echo base_path.'MyCP/'.$row['ad_image'] ?>" width="50" /></td>
                                <td valign="top"><?php echo $getmem['member_name'] ?></td>
                                <td valign="top"><?php echo $row['ad_title'] ?></td>
                                <td valign="top"><?php echo $row['ad_text'] ?></td>
                                <td valign="top"><?php echo ucfirst($row['ad_gender']) ?></td>
                                <td valign="top"><?php echo ucfirst($row['ad_status']) ?></td>
                                 <td valign="top"><?php echo FormatDateMonth($row['ad_startdate']).' - '.FormatDateMonth($row['ad_enddate']) ?></td>
                                <td valign="top">
                                    <!--<a href="<?php echo base_path ?>MyCP/ads-post.php?id=<?php echo base64_encode($row['id']); ?>&mode=edit"  class="inner-link" style="cursor:pointer;"> <img src="<?php echo base_path ?>MyCP/images/pencil.png" border="0" alt="Edit" /></a>-->
                                    <a href="<?php echo base_path ?>MyCP/ads-list.php?id=<?php echo $row['id']; ?>&mode=delete" onclick="return confirmDelete()" title="Delete"><img src="<?php echo base_path ?>MyCP/images/dd.gif" /></a>
                                </td>
