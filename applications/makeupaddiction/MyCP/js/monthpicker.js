...
jQuery.fn.monthpicker = function(opts,callback){
	...
	return this.each(function(){
		...
		jQuery("<table width=100%></table>").append($body).appendTo($container.html(""));
		...
		function add(element) {		
			...
			if (tpl.type == "dropdown") {
				...
				$body.append('<th width="3%" class="caption">' + tpl.caption + '</th>').append(jQuery("<td width=3%></td>").append($list));
	
			} else if (tpl.type == "button") {	
			
				var $items = jQuery('<td width="80%" class="month"></td>');
				...
				$body.append('<th width="3%" class="caption">' + tpl.caption + '</th>').append($items);
