 ...
 * 1. 
*/
class gdCreateMatchAccounttoProfile
    extends zAppBaseObject
{
    ...
    function createRecordMatchUseraccounttoProfile($usersafety_useraccount_uid,
                                                $usersafety_userprofile_uid)
    {
        ...
        $sqlstmnt = "INSERT INTO match_usersafety_useraccount_to_userprofile SET ".
            ...
            "usersafety_userprofile_uid=:usersafety_userprofile_uid";
        
        $appcon = new ZAppDatabase();
        $appcon->setApplicationDB("USERSAFETY");
        $appcon->setStatement($sqlstmnt);
        $appcon->bindParam(":usersafety_useraccount_uid", $usersafety_useraccount_uid);
        $appcon->bindParam(":usersafety_userprofile_uid", $usersafety_userprofile_uid);
        $appcon->execUpdate();
        if($appcon->getTransactionGood())
        {
            if($appcon->getRowCount() > 0)
            {
                $this->setResult_Record($appcon->getRowfromLastId($appcon, "match_usersafety_useraccount_to_userprofile", $appcon->getLastInsertID()));
        ...
        $this->gdlog()->LogInfoEndFUNCTION("createRecordMatchUseraccounttoProfile");
        return $fr;
    }
    
    function getUid() { return $this->getResult_RecordField("uid"); }
    function getUsersafetyUseraccountUid() { return $this->getResult_RecordField("usersafety_useraccount_uid"); }
    function getUsersafetyUserprofileUid() { return $this->getResult_RecordField("usersafety_userprofile_uid"); }
