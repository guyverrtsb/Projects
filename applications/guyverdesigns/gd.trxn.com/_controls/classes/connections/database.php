    ...
    extends ZGDDatabase
{
    ...
    {
        $this->gdlog()->LogInfo("bindParam()--{".$name."}-{".$value."}");
    ...
    public function execUpdate()
    {
        ...
        {
            $this->gdlog()->LogInfo("execUpdate():Good Transaction.");
        ...
        {
            $this->gdlog()->LogInfo("execUpdate():Failed Transaction:".$this->getStatement()->errorInfo());
    ...
    public function execSelect()
    {
        if($this->getStatement()->execute())
        {
            $this->gdlog()->LogInfo("execSelect():Good Transaction.");
            $this->setTransactionSuccessful();
            $this->setErrorContainer("NO_ERROR_CODE", "NO_ERROR_INFO",
                $this->getRowCount(), "-1");
        }
        ...
        {
            $this->gdlog()->LogInfoERROR("execSelect():Failed Transaction:".$this->getStatement()->errorInfo());
            $this->setTransactionFailure();
            $this->setErrorContainer($this->getStatement()->errorCode(), $this->getStatement()->errorInfo(),
                "-1", "-1");
        }
    ...
    public function getRowfromLastId($dbcontrol, $dbname, $lid)
    {
        ...
        {
            $this->gdlog()->LogInfo("getRowfromLastId():Good Transaction:".$dbname.":".$lid);
            $row = $dbcontrol->getStatement()->fetch(PDO::FETCH_ASSOC);
            return $row;
        }
        else
        {
            $this->gdlog()->LogInfo("getRowfromLastId():Failed Transaction:".$dbname);
        }
    }
    ...
    public function getRowfromLastUid($dbcontrol, $dbname, $uid)
    {
        ...
        if($dbcontrol->getRowCount() > 0)
        {
            $this->gdlog()->LogInfo("getRowfromLastUid():Good Transaction:".$dbname.":".$lid);
            $row = $dbcontrol->getStatement()->fetch(PDO::FETCH_ASSOC);
            return $row;
        }
        else
        {
            $this->gdlog()->LogInfo("getRowfromLastUid():Failed Transaction:".$dbname);
        }
    }
    ...
    public function rollbackcommit()
    {
        ...
        {
            $this->gdlog()->LogInfo("rollbackcommit():Good Commit");
            $this->getConnection()->commit();
        }
        else
        {
            $this->gdlog()->LogInfo("rollbackcommit():Bad Commit");
