...
<?php gdreqonce("/gd.trxn.com/gduploadmimes/_controls/classes/mimes/mimetodatabase.php"); ?>
<?php
/*
    ...
    extends zMimesBaseObject
{
    ...
    var $appl_table_name = "mimes_appl_standard_image";
    var $orig_table_name = "mimes_appl_standard_original_image";
    var $thmb_table_name = "mimes_appl_standard_thumbnail_image_100x100";
    
    function __construct($formfieldname = "GDUploadImageFile", 
                        $meta_table_name = "mimes_meta_standard", 
                        $appl_table_name = "mimes_appl_standard_image", 
                        $target_h_pxl = "500", 
                        $target_w_pxl = "500",
    ...
    function executeControl()
    {
        ...
        if($r == "VALID")
        {
            ...
            if($r == "FILE_UPLOADED_SUCCESSFULY")
            {
                ...
                {
                    $this->orig_mimes_uid = $zitdorig->getMimesMetaUid();
                    ...
                    if($r == "MIME_BLOB_REGISTERED")
                    {
                        ...
                        $return_mimes_data = $return_mimes_data.":ORIG:".$zitdorig->getMimesMetaUid();
                        
                        gdlog()->LogInfo("LOAD_TO DB_THUMBNAIL");
                        $zitdthmb = new zMimetoDatabase();
                        ...
                        if($r == "MIME_REGISTERED")
                        {
                            gdlog()->LogInfo("THUMBNAIL_MIME_DATA{".$zitdthmb->getMimesMetaUid()."}");
                            ...
                            if($r == "MIME_BLOB_REGISTERED")
                            {
                                ...
                                $return_mimes_data = $return_mimes_data.":THMB:".$zitdthmb->getMimesMetaUid();
                                
                                gdlog()->LogInfo("LOAD_TO DB_APPLICATION");
                                $zitdappl = new zMimetoDatabase();
                                ...
                                if($r == "MIME_REGISTERED")
                                {
                                    ...
                                    gdlog()->LogInfo("APPLICATION_MIME_DATA{".$zitdappl->getMimesMetaUid()."}");
                                    $r = $zitdappl->registerMimeBlobImage($applimg->getWidth(),
                                                                        $applimg->getHeight());
                                    if($r == "MIME_BLOB_REGISTERED")
                                    {
                                        gdlog()->LogInfo("APPLICATION_MIME_BLOB_UPLOADED{".$zitdappl->getMimesApplUid()."}");
                                        $return_mimes_data = $return_mimes_data.":APPL:".$zitdappl->getMimesMetaUid();
                                    }
                                }
                            }
                        }
