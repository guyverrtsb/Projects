    ...
    extends Z_GD_Database
{
    ...
    function authenticate($email, $password)
    {
        ...
        $dbcontrol->bindParam(":email", $email);
        ...
        if($dbcontrol->getTransactionGood())
        {
            if($dbcontrol->getRowCount() > 0)
            {
                ...
                if($row["active"] == "F")
                {
                    $dbcontrol->saveActivityLog("ACCOUNT_INACTIVE", ":UID:".$row["uid"].
                        ":".$row["email"].":");
                ...
                if($nolt >= 3)
                {
                    $dbcontrol->saveActivityLog("USER_IS_LOCKED_TOO_MANY_TRIES", ":UID:".$row["uid"].
                        ":".$row["email"].":");
                ...
                if($row["password"] == $password)
                {
                    ...
                    $dbcontrol->saveActivityLog("USER_LOGIN_SUCCESS", ":UID:".$row["uid"].
                        ":".$row["email"].":");
    ...
    function isAthenticated()
    {
        ...
        {
            $this->redirectToLogin("100",
                "NOT_LOGGED_IN",
                "User is not Logged In.  Please Login or Register");
        }
    }
