	...
	implements SectionIntrfc
{
	public void execute(String key)
	{
		...
		{
			this.setPreparedStatement("SELECT lid, uid, url, idx, profile, screendata FROM entity_scholarshipsource ");
			this.retrieve(key);
		}
		...
		else if(key.equalsIgnoreCase("GET_SCHOLARSHIPSOURCE_FROM_IDX"))
		{
			this.setPreparedStatement("SELECT lid, uid, url, idx, profile, screendata FROM entity_scholarshipsource where idx = ?");
			this.bind(1, this.getDataPassInt("ZZZZCOUNTER"));
			this.retrieve(key +  " [idx=" + Integer.toString(this.getDataPassInt("ZZZZCOUNTER")) + "]");
		}
		...
		else if(key.equalsIgnoreCase("UPDATE_SCHOLARSHIPSOURCEDATA"))
		{
			this.setPreparedStatement("UPDATE entity_scholarshipsourcedata SET " +
					"`changeddt` = NOW(), " +
					"`idx` = ?, " +
					"`screendata_json` = NULL " +
					"WHERE `entity_scholarshipsource_uid` = ?");
			this.bind(1, this.getDataPassInt("ZZZZCOUNTER"));
			this.bind(2, this.getDataPassString("entity_scholarshipsource_uid"));
			this.create(key + " [entity_scholarshipsource_uid=" + this.getDataPassString("entity_scholarshipsource_uid") + "]");
		}
